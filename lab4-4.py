...
# Исходя из определения: МНОЖЕСТВО в Python -  это структура данных, содержащая не повторяющиеся элементы в случайном порядке.

b = set('hello')  # Преобразование строки в множество

print(b)  # Объясните результат
# Конструктор set() разбивает строку поэлементно и удаляет повторяющиеся
